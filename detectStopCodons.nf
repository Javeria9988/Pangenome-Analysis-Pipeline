...
def detect_premature_stop_codons(consensus_fasta, output_file, input_format, stop_codon_symbol):
    ...
    for record in alignment:
        ...
        # Debugging: Log the translated protein sequence
        print(f"Gene: {gene_name}, Translated Sequence: {protein_sequence}")
        
        stop_positions = []
        
        for i, aa in enumerate(protein_sequence):
            if aa == stop_codon_symbol and i < len(protein_sequence) - 1:  # Ensure it's a premature stop codon
                stop_codon_position = (i + 1) * 3
                stop_positions.append(stop_codon_position)
        
        if stop_positions:
            print(f"Gene: {gene_name}, Premature Stop Codon Positions: {stop_positions}")  # Debugging
